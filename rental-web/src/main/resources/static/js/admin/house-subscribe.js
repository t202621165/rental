...
var table = $('#data-table').DataTable({
        ...
        columnDefs: [{ // 定义表格样式
            ...
            render: function (data, type, row, meta) {
                return '<td class="text-l"><u style="cursor:pointer" class="text-primary"' +
                    'onClick="house_edit(\'查看\', \'/house/show?id=' + row.house.id + '\')" title="查看">' + data + '</u></td>';
            }
        }, {
            targets: 2,
            render: function (data, type, row, meta) {
                return '<td><img onClick="house_edit(\'查看\', \'/house/show?id=' + row.house.id + '\')" title="查看"' +
                    ' class="picture-thumb" src="http://q1ucvq5bt.bkt.clouddn.com/' + data + '?imageView2/1/w/200/h/100"></td>';
            }
        }, {
            targets: 7,
            render: function (data, type, row, meta) {
            ...
            render: function (data, type, row, meta) {
                return '<td class="f-14 td-manage"><a style="text-decoration:none" class="ml-5"' +
                    ' onClick="finishSubscribe(this,' + row.house.id + ', ' + row.id + ')" href="javascript:;"' +
...
function finishSubscribe(obj, houseId, rowId) {
